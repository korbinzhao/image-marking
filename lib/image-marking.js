...
var ImageMarking = function (_React$Component) {
  ...
  _createClass(ImageMarking, [{
    ...
    value: function shouldComponentUpdate(nextProps, nextState) {
      ...
      if (JSON.stringify(dataSource) === JSON.stringify(shapesData) && JSON.stringify(nextState) === JSON.stringify(this.state)) {
        return false;
      } else {
        return true;
      }
    ...
    value: function handleShapesData(shapesData) {
      ...
      return shapesData.filter(function (item) {
    ...
    value: function uniqueShapePoints(shapesData) {
      var arr1 = shapesData.map(function (item) {
        return item.join(',');
      });
